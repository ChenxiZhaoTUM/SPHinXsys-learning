...
// observer location
StdVec<Vecd> observation_location = {Vecd(Gate_xDis + 0.3102, Gate_height + 0.3102 * tan(10.0/360.0 * 2 * Pi))};
//----------------------------------------------------------------------
...
//----------------------------------------------------------------------
Real rho0_s = 1900;   /**< Reference density of gate. */
//Real rho0_s = 10000;   /**< Reference density of gate. */
//Real poisson = 0.3; /**< Poisson ratio. */
Real poisson = 0.48; /**< Poisson ratio. */
//Real Youngs_modulus = 1.0E10;
...
class Panel;
template <>
class ParticleGenerator<SurfaceParticles, Panel> : public ParticleGenerator<SurfaceParticles>
{
    Real Gate_length;
  ...
  public:
    explicit ParticleGenerator(SPHBody &sph_body, SurfaceParticles &surface_particles,
                               Real Gate_length, Real particle_spacing_gate, Real Gate_thickness)
        : ParticleGenerator<SurfaceParticles>(sph_body, surface_particles),
          Gate_length(Gate_length), particle_spacing_gate(particle_spacing_gate), Gate_thickness(Gate_thickness){};
    void prepareGeometricData() override
    {
        const auto particle_number_gate = int(Gate_length / particle_spacing_gate);
        // generate particles for the elastic gate
        for (int i = 0; i < particle_number_gate; i++)
        {
            Real x = Gate_xDis + Real(i) * particle_spacing_gate;
            Real y = Gate_height;
...
class DisControlGeometryLeft : public BodyPartByParticle
{
  public:
    DisControlGeometryLeft(SPHBody &body, const std::string &body_part_name)
        : BodyPartByParticle(body, body_part_name)
    {
        TaggingParticleMethod tagging_particle_method = std::bind(&DisControlGeometryLeft::tagManually, this, _1);
        tagParticles(tagging_particle_method);
    };
    virtual ~DisControlGeometryLeft(){};

  private:
    void tagManually(size_t index_i)
    {
        Vecd pos = base_particles_.ParticlePositions()[index_i];
        if (pos[0] < Gate_xDis + 25.0e-3 )
         //|| pos[0] > Gate_xDis + (25.0 + 495.0) * 1.0e-3)
        {
...
class ControlDisplacementLeft : public thin_structure_dynamics::ConstrainShellBodyRegion
{
  public:
    ControlDisplacementLeft(BodyPartByParticle &body_part)
        : ConstrainShellBodyRegion(body_part),
          vel_(particles_->getVariableDataByName<Vecd>("Velocity")),
          angular_vel_(particles_->getVariableDataByName<Vecd>("AngularVelocity")){};
    virtual ~ControlDisplacementLeft(){};
  ...
  protected:
    ...
    {
        vel_[index_i] = Vecd(0.0, - 4.0);
        angular_vel_[index_i] = Vecd::Zero();
    };
};

class DisControlGeometryRight : public BodyPartByParticle
{
  public:
    DisControlGeometryRight(SPHBody &body, const std::string &body_part_name)
        : BodyPartByParticle(body, body_part_name)
    {
        TaggingParticleMethod tagging_particle_method = std::bind(&DisControlGeometryRight::tagManually, this, _1);
        tagParticles(tagging_particle_method);
    };
    virtual ~DisControlGeometryRight(){};

  private:
    void tagManually(size_t index_i)
    {
        Vecd pos = base_particles_.ParticlePositions()[index_i];
        if (pos[0] > Gate_xDis + (25.0 + 495.0) * 1.0e-3)
        {
            body_part_particles_.push_back(index_i);
        }
    };
...
class ControlDisplacementRight : public thin_structure_dynamics::ConstrainShellBodyRegion
{
  public:
    ControlDisplacementRight(BodyPartByParticle &body_part)
        : ConstrainShellBodyRegion(body_part),
          vel_(particles_->getVariableDataByName<Vecd>("Velocity")),
          angular_vel_(particles_->getVariableDataByName<Vecd>("AngularVelocity")){};
    virtual ~ControlDisplacementRight(){};
...
int main(int ac, char *av[])
{
    ...
    SolidBody gate(sph_system, makeShared<DefaultShape>("Panel"));
    ...
    gate.generateParticles<SurfaceParticles, Panel>(Gate_length, dp_s, Gate_width);
    ...
    ContactRelationFromShellToFluid water_gate_contact(water_block, {&gate}, {true});
    ContactRelationFromFluidToShell gate_water_contact(gate, {&water_block}, {true});
    ...
    DisControlGeometryLeft dis_control_geometry_left(gate, "DisControlGeometryLeft");
    SimpleDynamics<ControlDisplacementLeft> dis_control_left(dis_control_geometry_left);
    DisControlGeometryRight dis_control_geometry_right(gate, "DisControlGeometryRight");
    SimpleDynamics<ControlDisplacementRight> dis_control_right(dis_control_geometry_right);
    ...
    Dynamics1Level<ComplexInteraction<fluid_dynamics::Integration1stHalf<Inner<>, Contact<Wall>, Contact<Wall>>, AcousticRiemannSolver, NoKernelCorrection>>
        pressure_relaxation(water_block_inner, water_wall_contact, water_gate_contact);
    Dynamics1Level<ComplexInteraction<fluid_dynamics::Integration2ndHalf<Inner<>, Contact<Wall>, Contact<Wall>>, AcousticRiemannSolver>>
        density_relaxation(water_block_inner, water_wall_contact, water_gate_contact);
    InteractionWithUpdate<fluid_dynamics::BaseDensitySummationComplex<Inner<FreeSurface>, Contact<>, Contact<>>>
        update_density_by_summation(water_block_inner, water_wall_contact, water_gate_contact);
    ...
    InteractionWithUpdate<ComplexInteraction<fluid_dynamics::ViscousForce<Inner<>, Contact<Wall>, Contact<Wall>>,
        ...
        viscous_acceleration(water_block_inner, water_wall_contact, water_gate_contact);

    InteractionWithUpdate<TwoBoundarySpatialTemporalFreeSurfaceIndicationComplex>
        free_stream_surface_indicator(water_block_inner, water_wall_contact, water_gate_contact);
    /** Impose transport velocity formulation. */
    InteractionWithUpdate<fluid_dynamics::TwoBoundaryTransportVelocityCorrectionComplex<BulkParticles>>
        transport_velocity_correction(water_block_inner, water_wall_contact, water_gate_contact);
    //----------------------------------------------------------------------
    //	Algorithms of FSI.
    //----------------------------------------------------------------------
    ...
    write_real_body_states_to_vtp.addToWrite<Matd>(gate, "MidSurfaceCauchyStress");
        ...
        while (integration_time < output_interval)
        {
            ...
            while (relaxation_time < Dt)
            {
                ...
                while (dt_s_sum < dt)
                {
                    ...
                    dis_control_left.exec(dt);
                    dis_control_right.exec(dt);
