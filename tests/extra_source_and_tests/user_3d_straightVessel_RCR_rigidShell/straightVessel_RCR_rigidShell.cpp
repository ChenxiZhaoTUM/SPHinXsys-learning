...
struct InflowVelocity
{
    ...
    Vecd operator()(Vecd &position, Vecd &velocity)
    {
        ...
        /*target_velocity[0] = 0.2;
        return run_time < 1 ? target_velocity * run_time : target_velocity;*/
    }
};
...
int main(int ac, char *av[])
{
    ...
    //std::cout << "Re = " << rho0_f * U_f * diameter / mu_f << std::endl;
    ...
    SolidBody shell_body(sph_system, makeShared<DefaultShape>("ShellBody"));
    ...
    //----------------------------------------------------------------------
    // shell dynamics
    InteractionDynamics<thin_structure_dynamics::ShellCorrectConfiguration> wall_corrected_configuration(shell_inner);
    ...
    InteractionWithUpdate<fluid_dynamics::DensitySummationFreeStreamComplex> update_fluid_density(water_block_inner, water_shell_contact);
    BodyAlignedBoxByParticle emitter(water_block, makeShared<AlignedBoxShape>(xAxis, Transform(Vec3d(left_bidirectional_translation)), bidirectional_buffer_halfsize));
    SimpleDynamics<fluid_dynamics::EmitterInflowInjection> emitter_inflow_injection(emitter, in_outlet_particle_buffer);
    BodyAlignedBoxByCell emitter_buffer(water_block, makeShared<AlignedBoxShape>(xAxis, Transform(Vec3d(left_bidirectional_translation)), bidirectional_buffer_halfsize));
    SimpleDynamics<fluid_dynamics::InflowVelocityCondition<InflowVelocity>> emitter_buffer_inflow_condition(emitter_buffer);
    BodyAlignedBoxByCell disposer(water_block, makeShared<AlignedBoxShape>(xAxis, Transform(Vec3d(right_bidirectional_translation)), bidirectional_buffer_halfsize));
