...
Real U_max = Re * mu_f / rho0_f / diameter;
...
int main(int ac, char *av[])
{
    ...
    if (system.RunParticleRelaxation() && !system.ReloadParticles())
    {
        ...
        BodyAlignedBoxByCell inlet_detection_box(shell_boundary,
                                             makeShared<AlignedBoxShape>(xAxis, Transform(Rotation3d(Pi, Vecd(0., 1.0, 0.)), Vec3d(-emitter_halfsize[0], 0., 0.)), emitter_halfsize));
        BodyAlignedBoxByCell outlet_detection_box(shell_boundary,
                                                makeShared<AlignedBoxShape>(xAxis, Transform(Vec3d(full_length + emitter_halfsize[0], 0., 0.)), disposer_halfsize));

        RealBody test_body_in(
        system, makeShared<AlignedBoxShape>(xAxis, Transform(Rotation3d(Pi, Vecd(0., 1.0, 0.)), Vec3d(-emitter_halfsize[0], 0., 0.)), emitter_halfsize, "TestBodyIn"));
        test_body_in.generateParticles<BaseParticles, Lattice>();

        RealBody test_body_out(
        system, makeShared<AlignedBoxShape>(xAxis, Transform(Vec3d(full_length + emitter_halfsize[0], 0., 0.)), disposer_halfsize, "TestBodyOut"));
