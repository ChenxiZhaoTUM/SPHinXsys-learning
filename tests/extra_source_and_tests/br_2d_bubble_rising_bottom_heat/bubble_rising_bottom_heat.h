/**
 * @file    bubble_rising_bottom_heat.h
 * @brief   2-D bubble rising benchmark with bottom/top thermal convection-diffusion and buoyancy.
 */
...
// Zero flux on the left/right side walls.
Real heat_flux = 0.0;
...
class SideNeumannWall : public MultiPolygonShape
{
  public:
    explicit SideNeumannWall(const std::string &shape_name) : MultiPolygonShape(shape_name)
    {
...
class DirichletWallBoundaryInitialCondition : public LocalDynamics
{
  public:
    ...
    void update(size_t index_i, Real dt)
    {
        phi_[index_i] = (pos_[index_i][1] < 0.5 * DH)
                            ? bottom_temperature
                            : top_temperature;
    }
...
// Order:
// inner, fluid-fluid contact, bottom/top Dirichlet, left/right Neumann.
//----------------------------------------------------------------------
