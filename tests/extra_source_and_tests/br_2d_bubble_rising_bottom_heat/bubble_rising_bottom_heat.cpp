/**
 * @file    bubble_rising_bottom_heat.cpp
 * @brief   2-D bubble rising simulation with bottom/top thermal convection-diffusion and buoyancy.
 */
#include "bubble_rising_bottom_heat.h"
...
int main(int ac, char *av[])
{
    ...
    SolidBody side_Neumann(sph_system, makeShared<SideNeumannWall>("SideNeumann"));
    side_Neumann.defineMaterial<Solid>();
    side_Neumann.generateParticles<BaseParticles, Lattice>();
    ...
    ContactRelation liquid_contact_side_Neumann(liquid_body, {&side_Neumann});
    ...
    ContactRelation bubble_contact_side_Neumann(bubble_body, {&side_Neumann});
    ...
    SimpleDynamics<NormalDirectionFromBodyShape> side_Neumann_normal_direction(side_Neumann);
    ...
    SimpleDynamics<DirichletWallBoundaryInitialCondition>
        setup_bottom_top_Dirichlet_initial_condition(no_slip_wall);
    SimpleDynamics<NeumannWallBoundaryInitialCondition>
        setup_side_Neumann_initial_condition(side_Neumann);
    ...
    write_states.addToWrite<Real>(no_slip_wall, diffusion_species_name);
    ...
    setup_bottom_top_Dirichlet_initial_condition.exec();
    setup_side_Neumann_initial_condition.exec();

    liquid_normal_direction.exec();
    bubble_normal_direction.exec();
    wall_normal_direction.exec();
    no_slip_wall_normal_direction.exec();
    side_Neumann_normal_direction.exec();
