...
Real U_f = 130 * 1.0e-6 / (Pi * pow(fluid_radius, 2));
...
struct InflowVelocity
{
    ...
    Vec3d operator()(Vec3d &position, Vec3d &velocity)
    {
        ...
        target_velocity[0] = SMAX(130 * sin(Pi * t_in_cycle) * 1.0e-6 / (Pi * pow(fluid_radius, 2)), 0.0);
...
int main(int ac, char *av[])
{
    ...
    //shell_boundary.defineMaterial<LinearElasticSolid>(1e3, 1e6, 0.45);
