...
Real Re = 50;
Real mu_f = sqrt(rho0_f * fluid_radius * fluid_radius * fabs(Inlet_pressure - Outlet_pressure) / (4 * Re));   /**< Viscosity. */
//Real mu_f = 0.00025;
/**< Characteristic velocity. Average velocity */
Real U_f = fabs(Inlet_pressure - Outlet_pressure) *  fluid_radius * fluid_radius / (8 * mu_f * full_length);
Real U_max = 3.0 * U_f;  // parabolic inflow, Thus U_max = 2*U_f
...
{
    std::cout << "U_f = " << U_f << std::endl; //0.00220971
    std::cout << "mu_f = " << mu_f << std::endl;  //0.000353553
    //----------------------------------------------------------------------
    //  Define water shape
    //----------------------------------------------------------------------
