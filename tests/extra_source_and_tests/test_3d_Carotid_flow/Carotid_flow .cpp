...
int main(int ac, char *av[])
{
    ...
    BodyAlignedBoxByCell disposer_outlet_1(water_block, makeShared<AlignedBoxShape>
    ...
    SimpleDynamics<fluid_dynamics::DisposerOutflowDeletion> disposer_outflow_outlet_1(disposer_outlet_1, xAxis);
    BodyAlignedBoxByCell disposer_outlet_2(water_block, makeShared<AlignedBoxShape>
    ...
    SimpleDynamics<fluid_dynamics::DisposerOutflowDeletion> disposer_outflow_outlet_2(disposer_outlet_2, xAxis);
    ...
    BodyAlignedBoxByCell inflow_emitter(
        water_block, makeShared<AlignedBoxShape>(Transform(Rotation3d(std::acos(Eigen::Vector3d::UnitX().dot(normal_vector_inlet)),vector_inlet),
            Vecd(buffer_translation_inlet)),buffer_halfsize_inlet));
    fluid_dynamics::NonPrescribedPressureBidirectionalBuffer inflow_injection(inflow_emitter, xAxis, 10);
    BodyAlignedBoxByCell outflow_emitter_1(
        water_block, makeShared<AlignedBoxShape>(Transform(Rotation3d(std::acos(Eigen::Vector3d::UnitX().dot(-normal_vector_1)),-vector_1),
            Vecd(buffer_translation_1)),buffer_halfsize_1));
    fluid_dynamics::BidirectionalBuffer<OutflowPressure> outflow_injection_1(outflow_emitter_1, xAxis, 10);
    BodyAlignedBoxByCell outflow_emitter_2(
        water_block, makeShared<AlignedBoxShape>(Transform(Rotation3d(std::acos(Eigen::Vector3d::UnitX().dot(-normal_vector_2)), -vector_2),
            Vecd(buffer_translation_2)),buffer_halfsize_2));
    fluid_dynamics::BidirectionalBuffer<OutflowPressure> outflow_injection_2(outflow_emitter_2, xAxis, 10);
    /** output parameters */
    ...
    SimpleDynamics<fluid_dynamics::PressureCondition<OutflowPressure>> outflow_pressure_condition_1(outflow_emitter_1);
    SimpleDynamics<fluid_dynamics::PressureCondition<OutflowPressure>> outflow_pressure_condition_2(outflow_emitter_2);
    ...
    while (GlobalStaticVariables::physical_time_ < end_time)
    {
        ...
        {  
            ...
            while (relaxation_time < Dt)
            {
                ...
                outflow_pressure_condition_1.exec(dt); 
                outflow_pressure_condition_2.exec(dt);           
            ...
            disposer_outflow_outlet_1.exec();
            disposer_outflow_outlet_2.exec();
