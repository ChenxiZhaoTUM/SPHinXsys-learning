...
Real R2 = 1.212e8;
Real C = 1.5e-10;
Real radius_out_up = DW_out_up / 2;
Real radius_out_down = DW_out_down / 2;
...
int main(int ac, char *av[])
{
    ...
    ReduceDynamics<fluid_dynamics::AverageFlowRate<fluid_dynamics::TotalVelocityNormVal>> compute_right_up_flow_rate(right_up_emitter, Pi * pow(radius_out_up, 2));
    fluid_dynamics::BidirectionalBufferWindkessel<fluid_dynamics::RCRPressure> right_up_emitter_inflow_injection(right_up_emitter, in_outlet_particle_buffer, R1, R2, C);
    ...
    ReduceDynamics<fluid_dynamics::AverageFlowRate<fluid_dynamics::TotalVelocityNormVal>> compute_right_down_flow_rate(right_down_emitter, Pi * pow(radius_out_down, 2));
    fluid_dynamics::BidirectionalBufferWindkessel<fluid_dynamics::RCRPressure> right_down_emitter_inflow_injection(right_down_emitter, in_outlet_particle_buffer, R1, R2, C);
    ...
    SimpleDynamics<fluid_dynamics::WindkesselCondition<fluid_dynamics::RCRPressure>> right_up_inflow_pressure_condition(right_up_emitter, R1, R2, C);
    SimpleDynamics<fluid_dynamics::WindkesselCondition<fluid_dynamics::RCRPressure>> right_down_inflow_pressure_condition(right_down_emitter, R1, R2, C);
    ...
    while (GlobalStaticVariables::physical_time_ < end_time)
    {
        ...
        while (integration_time < Output_Time)
        {
            ...
            while (relaxation_time < Dt)
            {
                ...
                compute_right_up_flow_rate.exec();
                compute_right_down_flow_rate.exec();
                right_up_inflow_pressure_condition.getTargetPressure()->accumulateFlow(dt);
                right_down_inflow_pressure_condition.getTargetPressure()->accumulateFlow(dt);
                ...
                {
                    right_up_inflow_pressure_condition.getTargetPressure()->setInitialQPre();
                    right_down_inflow_pressure_condition.getTargetPressure()->setInitialQPre();
