...
class SphBasicGeometrySetting
{
  protected:
    ...
    // Single-objective target height for the fast-rising task.
    Real target_height = 1.5;
    ...
    // Bottom-heated configuration:
    //   bottom boundary: controllable hot Dirichlet wall
    //   top boundary: fixed cold Dirichlet wall
    //   left/right boundaries: no-slip, zero heat-flux Neumann walls
    Real bottom_temperature = 1.0;
    Real top_temperature = 0.0;
    Real delta_T = bottom_temperature - top_temperature;
    Real heat_flux = 0.0;
    Real reference_temperature = 0.5 * (bottom_temperature + top_temperature);
    ...
    //--------------------------------------------------------------------------
    // RL bottom-wall control setting.
    // Bottom wall is split into 5 horizontal x-segments.
    // Mean temperature is normally constrained to 1.0.
    //--------------------------------------------------------------------------
    inline static StdVec<Real> bottom_wall_segment_T = StdVec<Real>(5, 1.0);
    ...
    std::vector<Vecd> createLeftWall()
    {
    ...
    std::vector<Vecd> createRightWall()
    {
  ...
  public:
    static void setBottomWallSegmentTemperatures(
        ...
        Real mean_temperature = 1.0)
    {
        ...
        bottom_wall_segment_T = Ts;

        if (enforce_mean)
        {
            Real sum_T = 0.0;
            for (Real T : bottom_wall_segment_T)
            {
                sum_T += T;
            }

            Real current_mean = sum_T / Real(bottom_wall_segment_T.size());
    ...
    static void setBottomWallSegmentActions(
        ...
        Real mean_temperature = 1.0)
    {
        ...
        setBottomWallSegmentTemperatures(temperatures, false, mean_temperature);
    }
};
...
class NoSlipWall : public MultiPolygonShape, public SphBasicGeometrySetting
{
  public:
    explicit NoSlipWall(const std::string &shape_name)
        : MultiPolygonShape(shape_name)
    {
        // Left and right side walls are no-slip and zero heat-flux.
        multi_polygon_.addAPolygon(createLeftWall(), ShapeBooleanOps::add);
        multi_polygon_.addAPolygon(createRightWall(), ShapeBooleanOps::add);
    }
};

class BottomDirichletWall : public MultiPolygonShape, public SphBasicGeometrySetting
{
  public:
    explicit BottomDirichletWall(const std::string &shape_name)
        : MultiPolygonShape(shape_name)
    {
...
class TopDirichletWall : public MultiPolygonShape, public SphBasicGeometrySetting
{
  public:
    explicit TopDirichletWall(const std::string &shape_name)
        : MultiPolygonShape(shape_name)
    {
      ...
      public SphBasicGeometrySetting
{
  public:
    ...
    void update(size_t index_i, Real dt)
    {
        ...
        phi_[index_i] =
            bottom_temperature +
            (top_temperature - bottom_temperature) * y / DH;
    }
      ...
      public SphBasicGeometrySetting
{
  public:
    ...
    void update(size_t index_i, Real dt)
    {
        ...
        //----------------------------------------------------------------------
        // Top wall: fixed cold wall.
        //----------------------------------------------------------------------
        if (y > 0.5 * DH)
        {
            phi_[index_i] = top_temperature;
            return;
        }

        //----------------------------------------------------------------------
        // Bottom wall: segmented controlled hot wall.
        // x in [0, DL] is divided into bottom_wall_segment_T.size() segments.
        //----------------------------------------------------------------------
        const size_t n = bottom_wall_segment_T.size();
        if (n == 0)
        {
            phi_[index_i] = bottom_temperature;
            return;
        }

        // Clamp wall ghost-particle coordinates into physical width.
        x = SMAX(Real(0), SMIN(x, DL));

        Real eta = x / DL; // eta in [0, 1]
        ...
        phi_[index_i] = bottom_wall_segment_T[k];
    }
...
// Order:
// inner, fluid-fluid contact, bottom Dirichlet, top Dirichlet, side-wall Neumann.
//==============================================================================
