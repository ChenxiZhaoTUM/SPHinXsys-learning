/**
 * @file 	case_2d_channel_cVIcPO_shell.cpp
 * @brief 
 * @details
 * @author 
 */
...
Real scale = 0.001;
Real Lin = 20 * scale;
Real Lflex = 300 * scale;
Real Lout = 20 * scale;
Real DL = Lin + Lflex + Lout;             /**< Channel length. */
Real DH = 20.0 * scale;             /**< Channel height. */
Real resolution_ref = DH / 30.0; /**< Initial reference particle spacing. */
Real resolution_shell = resolution_ref;
Real wall_thickness = 2.5 * scale; /**< Extending width for BCs. */
StdVec<Vecd> observer_location = {Vecd(0.5 * DL, 0.5 * DH)}; /**< Displacement observation point. */
BoundingBox system_domain_bounds(Vecd(-wall_thickness, -wall_thickness - 0.5 * DH), Vecd(DL + wall_thickness, wall_thickness + 0.5 * DH));
//----------------------------------------------------------------------
//	Geometry parameters for boundary condition.
//----------------------------------------------------------------------
Vecd bidirectional_buffer_halfsize(resolution_ref * 2, 0.5 * DH);
Vecd emitter_translation = Vecd(resolution_ref * 2, 0.0);
Vecd disposer_translation(DL - 2.0 * resolution_ref, 0.0);
//----------------------------------------------------------------------
//	Material parameters.
//----------------------------------------------------------------------
...
Real rho0_s = 2210;           /** Normalized density. */
Real Youngs_modulus = 1.13e6; /** Normalized Youngs Modulus. */
Real poisson = 0.403;          /** Poisson ratio. */
  ...
  public:
    ...
    void prepareGeometricData() override
    {
        ...
        for (int i = 0; i < particle_number_mid_surface; i++)
        {
            ...
            // upper wall
            Real y1 = 0.5 * DH + 0.5 * resolution_shell_;
            ...
            // lower wall
            Real y2 = -0.5 * DH - 0.5 * resolution_shell_; // lower wall
...
struct InflowVelocity
{
    ...
    Vecd operator()(Vecd &position, Vecd &velocity, Real current_time)
    {
        ...
        Real Q_ave = 0.970659;
        Real a[8] = {0.880024, -1.515271, -0.387686, -0.231449, -0.017184, -0.032440,  0.001618, -0.018846};
        Real b[8] = {1.441506,  0.319259,  0.033648, -0.190892, -0.147866, -0.000713,  0.003940, -0.022032};

        Real w = 2 * Pi / interval_;
        Real Q = Q_ave;
        for (size_t i = 0; i < 8; i++)
        {
            Q += a[i] * cos(w * (i + 1) * t_in_cycle) + b[i] * sin(w * (i + 1) * t_in_cycle);
        }

        u_ave = 4 *  Q * 0.211E-3/60 / (Pi * (DH)/2 * (DH/2));

        target_velocity[0] = 1.5 * u_ave * (1.0 - (position[1] * position[1]) / (0.5 * DH) / (0.5 * DH));
...
StdVec<Vecd> createAxialObservationPoints(
    double full_length, Vecd translation = Vecd(0.0, 0.0))
{
...
StdVec<Vecd> createWallAxialObservationPoints(
    double full_length, Vecd translation = Vecd(0.0, 0.5 * DH))
{
    StdVec<Vecd> observation_points;
    int nx = 51;
    for (int i = 0; i < nx; i++)
...
StdVec<Vecd> displacement_observation_location = {
    Vecd(0.25 * DH, DH + 0.5 * resolution_shell) };
...
class BoundaryGeometry : public BodyPartByParticle
{
  ...
  private:
    ...
    {
        if (base_particles_.ParticlePositions()[index_i][0] < Lin 
            || base_particles_.ParticlePositions()[index_i][0] > DL - Lout)
        {
