...
Vec2d buffer_halfsize = Vec2d(2.0 * resolution_ref, tube_radius);
Vec2d buffer_translation = Vec2d(-0.5 * DL - BW + 2.0 * resolution_ref, 0.25);
Vec2d buffer_right_translation = Vec2d(0.5 * DL + BW - 2.0 * resolution_ref, 0.25);
//----------------------------------------------------------------------
...
//----------------------------------------------------------------------
std::vector<Vecd> createInnerWaterBlockShape()
{
    std::vector<Vecd> inner_water_block_shape;
    inner_water_block_shape.push_back(Vecd(-0.5 * DL - BW, 0.2));
    inner_water_block_shape.push_back(Vecd(0.5 * DL + BW, 0.2));
    inner_water_block_shape.push_back(Vecd(0.5 * DL + BW, 0.3));
    inner_water_block_shape.push_back(Vecd(-0.5 * DL - BW, 0.3));
    inner_water_block_shape.push_back(Vecd(-0.5 * DL - BW, 0.2));

    return inner_water_block_shape;
}

std::vector<Vecd> createInnerWallBoundaryShape()
{
    std::vector<Vecd> inner_wall_boundary_shape;
    inner_wall_boundary_shape.push_back(Vecd(-0.5 * DL, 0.0));
    inner_wall_boundary_shape.push_back(Vecd(0.5 * DL, 0.0));
    inner_wall_boundary_shape.push_back(Vecd(0.5 * DL, DH));
    inner_wall_boundary_shape.push_back(Vecd(-0.5 * DL, DH));
    inner_wall_boundary_shape.push_back(Vecd(-0.5 * DL, 0.0));

    return inner_wall_boundary_shape;
}

std::vector<Vecd> createOuterWallBoundaryShape()
{
    std::vector<Vecd> outer_wall_boundary_shape;
    outer_wall_boundary_shape.push_back(Vecd(-0.5 * DL - BW, - BW));
    outer_wall_boundary_shape.push_back(Vecd(0.5 * DL + BW, - BW));
    outer_wall_boundary_shape.push_back(Vecd(0.5 * DL + BW, DH + BW));
    outer_wall_boundary_shape.push_back(Vecd(-0.5 * DL - BW, DH + BW));
    outer_wall_boundary_shape.push_back(Vecd(-0.5 * DL - BW, - BW));

    return outer_wall_boundary_shape;
}

class TubeBlock : public MultiPolygonShape
{
public:
    explicit TubeBlock(const std::string &shape_name) : MultiPolygonShape(shape_name)
    {
        std::vector<Vecd> outer_tube_wall_shape;
        outer_tube_wall_shape.push_back(Vecd(-0.5 * DL - BW, 0.2 - BW));
        outer_tube_wall_shape.push_back(Vecd(0.5 * DL + BW, 0.2 - BW));
        outer_tube_wall_shape.push_back(Vecd(0.5 * DL + BW, 0.3 + BW));
        outer_tube_wall_shape.push_back(Vecd(-0.5 * DL - BW, 0.3 + BW));
        outer_tube_wall_shape.push_back(Vecd(-0.5 * DL - BW, 0.2 - BW));

        multi_polygon_.addAPolygon(outer_tube_wall_shape, ShapeBooleanOps::add);
        multi_polygon_.addAPolygon(createInnerWaterBlockShape(), ShapeBooleanOps::sub);
    }

};

class InnerFluidBlock : public MultiPolygonShape
{
  public:
    explicit InnerFluidBlock(const std::string &shape_name) : MultiPolygonShape(shape_name)
    {
        multi_polygon_.addAPolygon(createInnerWaterBlockShape(), ShapeBooleanOps::add);
    }
};

class OuterFluidBlock : public MultiPolygonShape
{
  public:
    explicit OuterFluidBlock(const std::string &shape_name) : MultiPolygonShape(shape_name)
    {
        std::vector<Vecd> outer_fluid_block_shape;
        outer_fluid_block_shape.push_back(Vecd(-0.5 * DL, 0.0));
        outer_fluid_block_shape.push_back(Vecd(0.5 * DL, 0.0));
        outer_fluid_block_shape.push_back(Vecd(0.5 * DL, LH));
        outer_fluid_block_shape.push_back(Vecd(-0.5 * DL, LH));
        outer_fluid_block_shape.push_back(Vecd(-0.5 * DL, 0.0));
        multi_polygon_.addAPolygon(outer_fluid_block_shape, ShapeBooleanOps::add);
        ...
        outer_tube_wall_shape.push_back(Vecd(-0.5 * DL - BW, 0.3 + BW));
        outer_tube_wall_shape.push_back(Vecd(-0.5 * DL - BW, 0.2 - BW));
        multi_polygon_.addAPolygon(outer_tube_wall_shape, ShapeBooleanOps::sub);
    }
};
...
class WallBoundary : public MultiPolygonShape
{
  public:
    ...
    {
        multi_polygon_.addAPolygon(createOuterWallBoundaryShape(), ShapeBooleanOps::add);
        multi_polygon_.addAPolygon(createInnerWallBoundaryShape(), ShapeBooleanOps::sub);
        std::vector<Vecd> outer_tube_wall_shape;
        outer_tube_wall_shape.push_back(Vecd(-0.5 * DL - BW, 0.2 - BW));
        outer_tube_wall_shape.push_back(Vecd(0.5 * DL + BW, 0.2 - BW));
        outer_tube_wall_shape.push_back(Vecd(0.5 * DL + BW, 0.3 + BW));
...
int main(int ac, char *av[])
{
    ...
    //tube.defineClosure<Solid, IsotropicThermalDiffusion>(Solid(), ConstructArgs(diffusion_species_name, k_tube, rho0_tube, Cp_tube));
    ...
    fluid_in.generateParticlesWithReserve<BaseParticles, Lattice>(in_outlet_particle_buffer);
    ...
    BodyAlignedBoxByCell left_emitter(fluid_in, makeShared<AlignedBoxShape>(xAxis, Transform(Vec2d(buffer_translation)), buffer_halfsize));
    fluid_dynamics::BidirectionalBuffer<LeftInflowPressure> left_bidirection_buffer(left_emitter, in_outlet_particle_buffer);
    BodyAlignedBoxByCell right_emitter(fluid_in, makeShared<AlignedBoxShape>(xAxis,Transform(Rotation2d(Pi), Vec2d(buffer_right_translation)), buffer_halfsize));
