...
int main(int ac, char* av[])
{
	...
	baffle.defineParticlesAndMaterial<ElasticSolidParticles, SaintVenantKirchhoffSolid>(rho0_s2, Youngs_modulus2, poisson2);
	...
	ContactRelation water_block_contact(water_block, { &tank, &baffle });
	ContactRelation air_block_contact(air_block, { &tank, &baffle });
	...
	InteractionDynamics<InterpolatingAQuantity<Vecd>>
		interpolation_observer_position_baffle(baffle_observer_contact, "Position", "Position");
	...
	solid_dynamics::AverageVelocityAndAcceleration baffle_average_velocity_and_acceleration(baffle);
	...
	InteractionWithUpdate<CorrectedConfigurationInner> baffle_corrected_configuration(baffle_inner);
	/** Time step size of elastic body. */
	ReduceDynamics<solid_dynamics::AcousticTimeStepSize> tank_acoustic_time_step(tank);
	ReduceDynamics<solid_dynamics::AcousticTimeStepSize> baffle_acoustic_time_step(baffle);
	...
	Dynamics1Level<solid_dynamics::Integration1stHalfPK2> baffle_stress_relaxation_1st_half(baffle_inner);
	...
	write_baffle_move.writeToFile(0);
	write_baffle_nom.writeToFile(0);
	...
	while (GlobalStaticVariables::physical_time_ < End_Time)
	{
		...
		while (integration_time < D_Time)
		{
			...
			while (relaxation_time < Dt)
			{
				...
				interpolation_observer_position_baffle.exec();
				...
				while (dt_s_sum < dt)
				{
					Real dt_s = SMIN(tank_acoustic_time_step.exec(), baffle_acoustic_time_step.exec(),  dt - dt_s_sum);
