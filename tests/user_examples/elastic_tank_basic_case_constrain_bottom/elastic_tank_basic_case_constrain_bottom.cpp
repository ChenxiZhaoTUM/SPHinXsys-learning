...
* @author
*/
#include "elastic_tank_basic_case_constrain_bottom.h"
...
int main(int ac, char* av[])
{
	...
	Vecd holder_halfsize(0.5*h, 0.5*h, 0.5*h);
	BodyRegionByParticle holder(tank,
                            makeShared<TransformShape<GeometricShapeBox>>(Transform(), holder_halfsize, "Holder"));
	...
	for (int i = 0; i != Dimensions; ++i)
	{
		...
		{
			ReduceDynamics<QuantityMomentOfInertia> compute_moment_of_inertia(tank, Vecd(0.0, 0.0, 0.0), i, j);
			moment_of_inertia(i, j) = compute_moment_of_inertia.exec();
		}
	}
	SimpleDynamics<Constrain3DSolidBodyRotation> constrain_rotation(tank, Vecd(0.0, 0.0, 0.0), moment_of_inertia);
