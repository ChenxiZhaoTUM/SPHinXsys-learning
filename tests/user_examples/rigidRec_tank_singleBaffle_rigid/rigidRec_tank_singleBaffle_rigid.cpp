...
* @author
*/
#include "rigidRec_tank_singleBaffle_rigid.h"
...
int main(int ac, char* av[])
{
	...
	baffle.defineAdaptationRatios(1.15, sph_system.resolution_ref_ / resolution_ref_solid);
    baffle.defineParticlesAndMaterial<SolidParticles, Solid>();
    baffle.generateParticles<ParticleGeneratorLattice>();

    ObserverBody pressure_observer(sph_system, "PressureObserver");
    pressure_observer.generateParticles<PressureObserverParticleGenerator>();

	//SolidBody E1_Probe_Shape(sph_system, makeShared<E1ProbeShape>("E1Shape"));
	//E1_Probe_Shape.defineParticlesAndMaterial<SolidParticles, Solid>();
 //   E1_Probe_Shape.generateParticles<ParticleGeneratorLattice>();

	//SolidBody E2_Probe_Shape(sph_system, makeShared<E2ProbeShape>("E2Shape"));
	//E2_Probe_Shape.defineParticlesAndMaterial<SolidParticles, Solid>();
 //   E2_Probe_Shape.generateParticles<ParticleGeneratorLattice>();
    ...
    ContactRelation pressure_observer_contact(pressure_observer, {&water_block});
 ...
 //   SimpleDynamics<solid_dynamics::FixBodyPartConstraint> constraint_baffle(baffle_fix_geo);
	...
	//--------------------------------------------------------------------------------
	BodyRegionByCell probe_E1(water_block, makeShared<E1ProbeShape>("PorbeE1"));
	BodyRegionByCell probe_E2(water_block, makeShared<E2ProbeShape>("PorbeE2"));
	BodyRegionByCell probe_E3(water_block, makeShared<E2ProbeShape>("PorbeE3"));
	BodyRegionByCell probe_E4(water_block, makeShared<E2ProbeShape>("PorbeE4"));
	ReducedQuantityRecording<ReduceDynamics<fluid_dynamics::FreeSurfaceHeight>>
		probe_surface_height_1(io_environment, probe_E1);
	ReducedQuantityRecording<ReduceDynamics<fluid_dynamics::FreeSurfaceHeight>>
		probe_surface_height_2(io_environment, probe_E2);
	ReducedQuantityRecording<ReduceDynamics<fluid_dynamics::FreeSurfaceHeight>>
		probe_surface_height_3(io_environment, probe_E3);
	ReducedQuantityRecording<ReduceDynamics<fluid_dynamics::FreeSurfaceHeight>>
		probe_surface_height_4(io_environment, probe_E4);

	ObservedQuantityRecording<Real> write_recorded_pressure("Pressure", io_environment, pressure_observer_contact);
	...
	while (GlobalStaticVariables::physical_time_ < End_Time)
	{
		...
		while (integration_time < D_Time)
		{
			...
			while (relaxation_time < Dt)
			{
				...
				if (GlobalStaticVariables::physical_time_ < 0.0)
                {
    ...
    //                constrain_rotation.exec();
			...
			pressure_observer_contact.updateConfiguration();
   ...
   //         baffle_contacts.updateConfiguration();
		...
		write_recorded_pressure.writeToFile();
