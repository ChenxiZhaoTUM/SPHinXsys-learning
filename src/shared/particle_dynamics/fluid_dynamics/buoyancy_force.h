  ...
  public:
    FirstLayerFromFluid(SPHBody &solid_body, SPHBody &fluid_body)
        : LocalDynamics(solid_body),
          ...
          solid_contact_indicator_(particles_->registerStateVariable<int>("SolidFirstLayerIndicator")),
          fluid_body_(fluid_body),
          spacing_ref_(solid_body.sph_adaptation_->ReferenceSpacing()){};
...
class FirstLayerFromFluids : public LocalDynamics
{
  public:
    FirstLayerFromFluids(SPHBody &solid_body, SPHBody &fluid_body, SPHBody &second_fluid_body)
        : LocalDynamics(solid_body),
          pos_(particles_->getVariableDataByName<Vecd>("Position")),
          solid_contact_indicator_(particles_->registerStateVariable<int>("SolidFirstLayerIndicator")),
          fluid_body_(fluid_body), second_fluid_body_(second_fluid_body),
          spacing_ref_(solid_body.sph_adaptation_->ReferenceSpacing()){};
    virtual ~FirstLayerFromFluids(){};

    void update(size_t index_i, Real dt = 0.0)
    {
        Real phi_1 = fluid_body_.getInitialShape().findSignedDistance(pos_[index_i]);
        Real phi_2 = second_fluid_body_.getInitialShape().findSignedDistance(pos_[index_i]);
        solid_contact_indicator_[index_i] = 1;
        if (phi_1 > 1.01 * spacing_ref_ && phi_2 > 1.01 * spacing_ref_)
            solid_contact_indicator_[index_i] = 0;
    }
...
class WallLocalNusseltNumberByFlux : public LocalDynamics, public DataDelegateContact
{
  public:
    explicit WallLocalNusseltNumberByFlux(BaseContactRelation &contact_relation, Real nu_coeff);
    virtual ~WallLocalNusseltNumberByFlux() {};
...
class ProjectionForNu : public LocalDynamics, public DataDelegateContact
{
  public:
    explicit ProjectionForNu(BaseContactRelation &contact_relation, Real nu_coeff);
    virtual ~ProjectionForNu() {};
